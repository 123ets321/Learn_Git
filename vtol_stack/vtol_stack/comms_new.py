...
from px4_msgs.msg import VehicleGlobalPosition, VehicleStatus, \
                         ...
                         PositionSetpointTriplet, PositionSetpoint
import math
import utm
...
class Comms(Node):
    def __init__(self,):
        ...
        # loads ghost parameters from a config file
...
#---------------------------------------------------------------------------------------- subscribers (px4 and gcs subscribers)
        ...
        self.vehicle_att_subscriber = self.create_subscription(VehicleAttitude, 
                                                               ...
                                                               self.vehicle_att_callback, qos_profile)
        ...
        self.lock_from_gcs_subscriber = self.create_subscription(String,
                                                                  "lock_from_gcs",
                                                                   self.lock_from_gcs_cb,
                                                                   10)
        
        self.drive_from_gcs_subscriber = self.create_subscription(String,
                                                                  "dive_from_gcs",
...
#---------------------------------------------------------------------------------------- publishers (to_px4 and other scripts publishers)
        ...
        self.publish_position_setpoint_triplet_publisher = self.create_publisher(PositionSetpointTriplet,"/fmu/in/position_setpoint_triplet",qos_profile)
        ...
        self.pixel_publisher = self.create_publisher(String,"pixel_values",10)
        ...
        self.command_publisher = self.create_publisher(String,"command_publisher",10)
...
#---------------------------------------------------------------------------------------- Load Params
    ...
    def load_ghost_params(self):
        ...
        self.is_mission_completed =  comms_gcs["is_mission_completed"]
    ...
    def dive_from_gcs_cb(self, msg):
        ...
        # to perform vision based dive and wade-off (both vbtg and coordinated strike)
        self.command_publisher.publish(self.dive_gcs)
        self.command = "dive"
        # print("stopped diving")
    ...
    def pixels_from_gcs_cb(self, msg):
        ...
        # send pixel values to vision script to filter the class
    ...
    def vehicle_global_position_callback(self, msg):
        ...
        if self.vehicle_global_position.lat and self.vehicle_global_position.lon and self.home_lat == 0.0:
            ...
            self.get_logger().info("Saved home lat and lon")
            print("saved alt : ", self.home_alt)
    ...
    def send_to_autopilot(self, data):
        if data["command"] == "takeoff":
            ...
            print("received a takeoff height of : ", data["takeoff_alt"], " ", self.takeoff_height)
            self.gate_opened = 1
            ...
            self.id = 0
            self.previous_waypoint = 0
            if len(self.all_previous_wp_ids) != 0: # new change
                self.previous_waypoint = self.all_previous_wp_ids[-1]
            self.update_waypoint(self.id, self.mission)
            # print("received mission")
            
        # elif data["command"] == "mission_upload":
        #     # handles pre-mission and strike as routine
        ...
        #         self.gate_opened = 1
        #         # print("opened gate in pre-mission")
        #     if self.command == "goto":
        #         self.current_waypoint = 0
        #         self.curr_wp_id = 0
        #         self.is_in_goto = False
        ...
        #         self.mission_completed = False
        #         self.is_mission_completed = False
        #     # check if dive or return routine is there
        ...
        #     # new waypoint projection logic
        #     if self.routine == "strike":
        #         print("\n\n routine : ", self.routine)
                
        #         if len(self.mission) > 1:
        #             last_but_one_wp = self.mission[self.mission_total - 2]
        #             self.command = "mission"
        #         else:
        #             self.command = "mission"
        #             # print("only dive is there....no wps are present....so taking home as previous wp")
                    
        #             # if the vehicle has already tookoff then vehicle's own position is the ref wp
        #             if self.arming_flag:
        #                 print("taking vehicle's own position as prev wp")
        #                 ref = {"coordinate":[self.vehicle_global_position.lon, self.vehicle_global_position.lat],
        #                         "id":0} 
        #             # else vehicle's home position is the ref wp
        #             else:
        #                 print("taking home as prev wp")
        #                 ref = {"coordinate":[self.home_lon, self.home_lat],
        #                         "id":0}
        #             last_but_one_wp = ref
        #         dive_wp = last_waypoint
        #         last_but_one_global_x, last_but_one_global_y = self.helper.latlon_to_m(last_but_one_wp["coordinate"][1], last_but_one_wp["coordinate"][0])
        #         dive_wp_global_x, dive_wp_global_y = self.helper.latlon_to_m(dive_wp["coordinate"][1], dive_wp["coordinate"][0])
                
        #         vec_lwp_to_dive = np.array([dive_wp_global_x - last_but_one_global_x, dive_wp_global_y - last_but_one_global_y, 0.0])
        #         mag_vec_lwp_to_dive = np.linalg.norm(vec_lwp_to_dive)
        #         print("magnitude of the vector : ", mag_vec_lwp_to_dive)

        #         if mag_vec_lwp_to_dive <= 800.0:
        #             # don't know if if have to normalize the vector
        #             vec_lwp_to_dive /= mag_vec_lwp_to_dive
        #             vec_lwp_to_dive *= 850.0
        #             virtual_global_lat, virtual_global_lon = self.helper.m_to_latlon(float(last_but_one_global_x + vec_lwp_to_dive[0]), float(last_but_one_global_y + vec_lwp_to_dive[1]))
        #             vec_lwp_to_dive_global = [float(virtual_global_lon), float(virtual_global_lat)]
        #             intermediate_virtual_wp = {"altitude":300.0,
        #                                        "coordinate":vec_lwp_to_dive_global,
        #                                        "id":last_but_one_wp["id"]+1,
        #                                        "routine":"waypoint",
        #                                        "status":False}
        #             self.mission.insert(self.mission_total - 1, intermediate_virtual_wp)
        #             self.mission[-1]["id"] += 1
        #             print("computed intermediate virtual waypoint : ", self.mission, "\n\n")

        #             # send the updated pre-mission to genga
...
#------------------------------------------------------------------------------------------------------ ETS
            ...
            self.mission = data["waypoint"]
            self.mission_total = len(self.mission)
            # NEW: Find strike waypoint dynamically
            self.strike_wp = next((wp for wp in self.mission if wp.get("routine") == "strike"), None)
            if self.strike_wp:
            ...
            if self.routine == "strike":
                ...
                # Get strike_wp and its index
                strike_index = next((i for i, wp in enumerate(self.mission) if wp.get("id") == self.strike_wp_id), None)
                ...
                if strike_index is not None and strike_index > 0:
                    ...
                    if self.arming_flag:
                        print("taking vehicle's own position as prev wp")
                        ref = {"coordinate":[self.vehicle_global_position.lon, self.vehicle_global_position.lat], "id":0}
                    else:
                        print("taking home as prev wp")
                        ref = {"coordinate":[self.home_lon, self.home_lat], "id":0}
                    last_but_one_wp = ref
                ...
                if mag_vec_lwp_to_dive <= 800.0:
                    ...
                    # Insert before strike WP
                    self.mission.insert(strike_index, intermediate_virtual_wp)
                    # Increment strike WP ID to keep mission unique
...
#------------------------------------------------------------------------------------------------------ ETS
            ...
            if self.mission != None:
                ...
                self.mission = data["waypoint"]
                # print("after update : ", self.mission)
                
                if self.is_mission_paused:
                    # new change (to test delete logic)
                    self.current_waypoint = 0
                    self.current_wp_id = 0
                    cmd, status = self.update_command_status()
                    for _ in range(5):
                    ...
                    if len(self.mission) > 0:
                        ...
                        if self.current_wp_id not in updated_wp_ids: 
                            # update the mission when resuming 
                            self.is_curr_wp_deleted = True
                            # self.current_waypoint = 0
                            # self.current_wp_id = 0
                        # if self.current_waypoint not in updated_wp_ids:
                        #     print("current waypoint has been deleted")
                        else:
                            # print("else part")
                            self.id = 0
                            self.previous_waypoint = 0
                        ...
                        # print("no wps....putting the vehicle to hold mode")
                        self.mission = None
            ...
            self.command = "pause"
            # print("pausing the mission.....enter into hold mode")
            ...
            if self.is_curr_wp_deleted: # new change
                self.id = 0
                self.previous_waypoint = 0
                self.mission_total = len(self.mission)
                self.update_waypoint(self.id, self.mission)
            ...
            # print("resuming the mission.....entering into offboard mode")
            
        elif data["command"] == "cancel":
            self.reset_mission_params(data["command"])
            # print("mission cancelled....erased mission waypoints....enter into loiter mode")            
            
        elif data["command"] == "strike_lat_long":
            # handles coordinated strike in fly page
            # print("stopped wp navigation (coordinated strike)")
            self.command = data["command"] 
            # print("received strike command : ", self.command, data)
            vehicle_global_x, vehicle_global_y = self.helper.latlon_to_m(self.vehicle_global_position.lat, self.vehicle_global_position.lon)
            dive_lat_x, dive_lon_y = self.helper.latlon_to_m(data["coordinate"][1], data["coordinate"][0])
            
            vec_vehicle_to_dive = np.array([dive_lat_x - vehicle_global_x, dive_lon_y - vehicle_global_y, 0.0])
            mag_vec_vehicle_to_dive = np.linalg.norm(vec_vehicle_to_dive)
            
            if mag_vec_vehicle_to_dive >= 800.0:
                # to perform coordinated strike (in fly page....not a pre-planned mission)
                # pass
                self.command_publisher.publish(self.command_from_gcs)
                print("published coordinated strike command immediately....magnitude : ", mag_vec_vehicle_to_dive)
            else:
                print("will have to compute a custom pre-mission....magnitude : ", mag_vec_vehicle_to_dive)
                vec_vehicle_to_dive /= mag_vec_vehicle_to_dive
                vec_vehicle_to_dive *= 850.0
                virtual_global_lat, virtual_global_lon = self.helper.m_to_latlon(float(vehicle_global_x + vec_vehicle_to_dive[0]), float(vehicle_global_y + vec_vehicle_to_dive[1]))
                vec_vehicle_to_dive_global = [float(virtual_global_lon), float(virtual_global_lat)]
                
                # [{'altitude': 50, 'coordinate': [75.70067579035182, 22.57685163308966], 'id': 1, 'routine': 'waypoint', 'status': False}, 
                #  {'altitude': 50, 'coordinate': [75.70253650190135, 22.584380271171042], 'id': 2, 'routine': 'waypoint', 'status': False}, 
                #  {'altitude': 120.0, 'coordinate': [75.70986741436784, 22.58082897086299], 'id': 3, 'routine': 'waypoint', 'status': False}, 
                #  {'altitude': 0, 'coordinate': [75.70847405336274, 22.581503993903354], 'id': 4, 'routine': 'strike', 'status': False}] 
                
                # create a custom pre-mission
    ...
    def update_waypoint(self, id, mission):
        ...
        # print(self.curr_alt)
        # print("updated current waypoint : ", wp_id)
    ...
    def reset_mission_params(self,cmd):
        ...
        self.is_mission_paused = False
        self.all_previous_wp_ids.clear() # new change
    ...
    def get_attitude(self):
        r, p, y = self.euler_from_quaternion(self.vehicle_att.q[1],self.vehicle_att.q[2],self.vehicle_att.q[3],self.vehicle_att.q[0])
        # if y < 0:
        #     y = 360 - y
    ...
    # def waypoints(self, speed0, position1, curr_lat, curr_lon, curr_alt):
    
    #     target_lat = curr_lat
    #     target_lon = curr_lon
    #     target_lon_m, target_lat_m, _, _ = utm.from_latlon(target_lat, target_lon)
    #     home_lon_m, home_lat_m, _, _ = utm.from_latlon(self.home_lat, self.home_lon)  
    ...
    def send_data_to_gcs(self, cmd, status):
        ...
        if self.st != None:
            ...
            flight_time = str(time.strftime("%H:%M:%S", time.gmtime(flight_time)))
        else:
        ...
        "mission_data": {
            ...
            "is_mission_completed": self.is_mission_completed,
    ...
    def takeoff_mc(self):
        ...
        print(f'Takeoff height: {self.takeoff_height}')
        self.publish_vehicle_command(
    ...
    def takeoff_vtol(self):
        self.get_logger().info('VTOL_takeoff')
        self.get_logger().info(self.takeoff_height)
        self.publish_vehicle_command(
            VehicleCommand.VEHICLE_CMD_NAV_VTOL_TAKEOFF,
            param1=0.0,
            param4=nan,
            param5=self.vehicle_global_position.lat,
            param6=self.vehicle_global_position.lon,
            param7=self.home_alt + self.takeoff_height)
        self.get_logger().info('VTOL_takeoff command sent')
    ...
    def change_loiter_altitude(self, alt):
        ...
        self.get_logger().info(f"increasing altitude with a give value of : {alt}")
    ...
    def timer_cb(self):
        ...
        if self.command != "strike_lat_long" and self.command != "dive":
            ...
            # print(f"sending nav heartbeat curr_command - {self.command} : ", time.time())
        ...
        # publish live data from autopilot to the gcs
        cmd, status = self.update_command_status()
        if self.home_flag:
            self.send_data_to_gcs(cmd, status)
        ...
        if self.gate_opened:
            ...
            if self.command == "mission" and self.mission != None and not self.is_mission_paused and self.vehicle_status.vehicle_type == VehicleStatus.VEHICLE_TYPE_FIXED_WING:
                ...
                if self.vehicle_status.nav_state !=VehicleStatus.NAVIGATION_STATE_OFFBOARD and not self.offboard_flag and not self.mission_completed:
                    ...
                    if self.routine != None:
                            ...
                            # strike routine (coordinate strike....when the mission is pre-planned)
                        if self.routine == "strike" :
                    ...
                    if self.helper.calculate_reached_threshold(self.vehicle_global_position.lat,
                        ...
                        self.id += 1
                        if self.id <= len(self.mission) - 1:
                            self.get_logger().info("updating wp")
                            self.update_waypoint(self.id, self.mission)
                        else:
                            self.get_logger().info("all wps reached")
                            self.id = 0
                            # detect mission completion waypoint
                            self.is_mission_completed = True
            ...
            elif self.command == "kill" and self.takeoff_flag and self.vehicle_status.nav_state != 18:
                self.change_mode(4.0, 5.0, 'Return')
                self.reset_mission_params(self.command)
